...
public class FormatAsPortable extends UiAutomatorTestCase 
{
	public void testPortableFormat() throws UiObjectNotFoundException, IOException, RemoteException
	{
...
UiWatcher okCancelWatcher = new UiWatcher() {
        ...
        if(Sd_card.exists())
        {
            ...
            {
            	System.out.println("Currently Sd card is formated as Internal, Formatting as Portable");
            	...
            	format_as_portable.clickAndWaitForNewWindow();
            	
            	// UiObject next=new UiObject(new UiSelector().text("Next"));
             	//UiObject unsupported=new UiObject(new UiSelector().text("Unsupported"));
                 UiObject Format=new UiObject(new UiSelector().className("android.widget.Button").text("Format"));
             	Format.clickAndWaitForNewWindow();
             	System.out.print("Erasing sdcard..");
             	
             	UiObject sdCardReady=new UiObject(new UiSelector().text("SD card is ready"));
            	...
            	{
            		System.out.println("SD card formatted as Portable Successfully");
            	}
            	else
            	{
            		assertTrue( "Failed to format as Internal",!format_as_portable.exists());
            	}
            }
        	else
        	{
        		System.out.println("Sdcard is already formatted as Portable");
        	}
        }
